...
//add a static method to parse n validate manufacturing date
	...
	// method
	public static String checkForDuplicate(String chasisNo, String colorString, Vehicle[] showroom) throws VehicleHandlingException {
		...
		// ctor
		Color color = parseAndValidateColor(colorString);
		Vehicle newVehicle = new Vehicle(chasisNo, color);
		for (Vehicle v : showroom)
			if (v != null)
				if (v.equals(newVehicle))
					throw new VehicleHandlingException("Dup Chasis with Color found !!!!");
	...
	public static Category parseAndValidateCategory(String categoryName) throws VehicleHandlingException {
		Category c = null;
		try {
			c = Category.valueOf(categoryName.toUpperCase());
		} catch(IllegalArgumentException e) {
			
			StringBuilder sb = new StringBuilder("Valid values are: ");
			for(Category cx : Category.values()) {
				sb.append(cx.name());
				sb.append(", ");
			}
			throw new VehicleHandlingException("Invalid Category Chosen! " + sb);
		}
		return c;
	}
	...
	public static Color parseAndValidateColor(String colorName) throws VehicleHandlingException {
		Color c = null;
		try {
			c = Color.valueOf(colorName.toUpperCase());
		} catch (IllegalArgumentException e) {
			StringBuilder sb = new StringBuilder("");
			sb.append("Valid values are: ");
			for(Color cx : Color.values()) {
				sb.append(cx.name());
				sb.append(", ");
			}
			System.out.println(sb);
			throw new VehicleHandlingException("Invalid Color Chosen! " + sb);
		}
		return c;
	}
}
